...
<div class="flex bg-bg gap-4 p-2 custom-border-bottom">
  ...
  <input
    type="search"
    class="w-1/2 p-2 rounded-md border"
    placeholder="Search"
  />
  <input
    type="select"
    class="w-1/4 p-2 rounded-md border"
    placeholder="Filter"
  />
  <button class="w-1/4 p-2 rounded-md bg-accent text-bg font-bold"
    >Add Client</button
  >
</div>
<div class="text-paragraph">
  {#each $clients as client}
    <div class="flex justify-between items-center custom-border-bottom">
      <button
        class="w-full p-2 text-left text-lg hover:bg-card {currentClient ===
        client.id
          ? 'bg-card'
          : ''}"
